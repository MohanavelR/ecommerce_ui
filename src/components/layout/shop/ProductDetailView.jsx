...
const ProductDetailView = ({ product,isLoading }) => {
    const [selectedImageIndex, setSelectedImageIndex] = useState(0);
    const [currentVariation,setCurrentVariation] =useState(null)
    const [isSeleted,setSelected]=useState({type:null,value:null})
    const [currentImage,setCurrentImage]=useState(null)
    // const [currentPrice,setCurrentPrice]=useState(product?.price?.current?product?.price?.current:0)
    // const [currentStock,setCurrentStock]=useState(product?.stock?product?.stock:0)
    // const [currentOffer,setCurrentOffer]=useState(product?.offer?product?.offer:0)
    // const [currentOriginal,setCurrentOriginal]=useState(product?.price?.original?product?.price?.original:0)
    const [variation,setVariation]=useState(null)
    const [currentSavedAmount,setCurrentSavedAmount]=useState(0)
    const isOutOfStock = currentVariation?.stock <= 0;
    const variationList=groupedVariations(Array.isArray(product?.variations) && product?.variations.length > 0 
    ? product.variations 
    : [])
    console.log(variationList)

// run when product is loaded
useEffect(() => {
...
function groupedVariations(variations = []) {
  ...
  function changeImage(index=0,image){
    ...
    // setCurrentPrice(product?.price?.current?product?.price?.current:0)
    // setCurrentStock(product?.stock?product?.stock:0)
   setCurrentImage(image)
//    setCurrentOriginal(product?.price?.original?product?.price?.original:0)
   setVariation(null)
//    setCurrentSavedAmount((product?.price?.current && product?.price?.original)?(product?.price?.original - product?.price?.current):0)
//    setCurrentOffer(product?.offer?product?.offer:0)
  }
    if(isLoading){
      return  <Loader/>
    } 
    if (!product) {
        // Render a graceful error or loading state if product data is missing
        return (
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 text-red-600 border border-red-300 bg-red-50 rounded-lg p-6">
                ...
                <p>Could not load product details. Please ensure a valid `product` object is passed to the component.</p>
            </div>
        );
    }
    // --------------------------------------------------------
    ...
    const isNewArrival = !isNaN(productCreationDate) && (Date.now() - productCreationDate < 7 * 24 * 60 * 60 * 1000);
    ...
    const tabContent = {
        Description: (
            <ul className="space-y-4">
                {Array.isArray(product?.description) && product?.description.map((desc, index) => <li key={index} className="text-gray-700">{desc}</li>)}
                {!Array.isArray(product?.description) && <li className="text-gray-500 italic">No description available.</li>}
            </ul>
        ),
        Features: (
            <ul className="list-disc list-inside text-gray-700 space-y-1">
                {Array.isArray(product?.features) && product?.features.map((feature, index) => <li key={index}>{feature}</li>)}
                {!Array.isArray(product?.features) && <li className="text-gray-500 italic">No features listed.</li>}
            </ul>
        ),
        "Additional Information": (
             <ul className="list-disc list-inside text-gray-700 space-y-1">
                {Array.isArray(product?.additionalInfo) && product?.additionalInfo.map((info, index) => <li key={index}>{info}</li>)}
                {!Array.isArray(product?.additionalInfo) && <li className="text-gray-500 italic">No specifications listed.</li>}
            </ul>
        ),
    };
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                        <div className="flex items-baseline">
                            ...
                            <span className="text-3xl font-bold text-gray-900">{currentVariation?.price?.currency}{currentVariation?.price?.current}</span>
                            {(currentVariation?.price?.original > 0) && currentSavedAmount  > 0 && (
                                <span className="ml-2 text-xl text-gray-500 line-through">{currentVariation?.price?.currency}{currentVariation?.price?.original}</span>
                            )}
                            {currentSavedAmount > 0 && (
                                <span className="ml-2 text-sm font-medium text-green-600">Save {product?.price?.currency}{currentSavedAmount}</span>
                            )}
                        </div>
                    ...
                    <div className="flex items-center">
                        ...
                        <span className={`text-sm font-medium ${isOutOfStock ? 'text-red-600' : 'text-green-600'}`}>
                            {isOutOfStock ? 'Out of Stock' : `In Stock (${currentVariation?.stock} units available)`}
                        </span>
                    </div>
                            ...
                            <div className="flex">
                                ...
                                <div className="ml-3">
                                  <p className="text-sm text-yellow-700">
                                    <span className="font-medium">Special Offer!</span> {currentVariation?.offer}
                                  </p>
                                </div>
                            </div>
                        </div>
                  ...
                  
{
  ...
  <div key={type}>
    ...
    <button
            ...
            disabled={(isSeleted.type==v.type && isSeleted.value==v.value)}
            className={`px-4 py-2 border disabled:cursor-default ${(isSeleted.type==v.type && isSeleted.value==v.value)?"border-2 bg-primary border-none  text-accent ":"text-gray-500 hover:bg-gray-50"} rounded-md  text-sm font-medium transition border-gray-300 `}
          >{v.value}</button>
    )}
    </div>
  </div>
                    ...
                    <div className="flex space-x-4">
                        ...
                        <button className="p-3 border border-gray-300 rounded-md hover:bg-gray-50" title="Add to Wishlist">
