...
import { useGetAllOrders, useGetOrderById, useUpdateOrderStatus } from '../../../store/order';
...
const AdminOrderDetail = ({ closeOrderDetailMethod, orderId }) => { // Keep orderId prop if SingleOrder uses it
  ...
  // State for the new status dropdown
  const [newOrderStatus, setNewOrderStatus] = useState('');
  const [isUpdating, setIsUpdating] = useState(false);

  // Set initial status when orderDetails loads/changes
  useEffect(() => {
  ...
  if (!orderDetails) return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <p className="bg-white p-6 rounded-lg shadow-2xl text-xl text-gray-700">Order not found.</p>
    </div>
  );
  ...
  const calculateTotals = (items) => {
    if (!items || items.length === 0) return { subtotal: 0, discount: 0, totalItems: 0 };
  ...
  const handleUpdateStatus = async () => {  
    dispatch(useUpdateOrderStatus({ orderId: _id, status: newOrderStatus })).then(res=>{
     if (res.payload?.success) {
       setMessageContextState({ ...messageContextState, is_show: true, text: res.payload?.message || `Order status updated to ${newOrderStatus}.`, success: true });
       dispatch(useGetAllOrders())
       dispatch(useGetOrderById(_id))
     } else {
       setMessageContextState({ ...messageContextState, is_show: true, text: res.payload?.message || "Failed to update order status.", success: false });
     }
 })  
  };


  // --- All possible order statuses (customize this list based on your backend) ---
          ...
          <div className="p-6 sm:p-8 lg:p-10 space-y-8">
            ...
            <div className="bg-indigo-50 shadow rounded-xl p-5 border-2 border-indigo-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <div className="flex flex-col">
                <h2 className="text-xl font-bold text-indigo-900">Manage Status</h2>
                <p className='text-sm text-indigo-700'>Current Status: <OrderStatusBadge status={orderStatus} /></p>
              </div>
            ...
            <header className="bg-white shadow rounded-xl px-6 py-5 border border-gray-200">
                <h1 className="text-2xl font-extrabold text-gray-900">
                    Order ID: <span className='text-indigo-600 font-mono'>#{_id.slice(-8).toUpperCase()}</span>
                </h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white shadow rounded-xl p-6 border border-gray-200 md:col-span-1">
                    <h2 className="text-lg font-bold text-gray-900 mb-4 border-b border-accent pb-2">Shipping Details</h2>
                    ...
                    <address className="not-italic text-gray-600 text-sm space-y-1 mt-1">
                        <p>{addressId.address}, {addressId.city} - {addressId.pincode}</p>
                ...
                <div className="bg-white shadow rounded-xl p-6 border border-gray-200 md:col-span-2">
                    <h2 className="text-lg font-bold text-gray-900 mb-4 border-accent border-b pb-2">Payment Details</h2>
                    <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
                        <div className='space-y-2'>
                           <DetailLine label="Method" value={paymentMethods} large={true} />
                           <DetailLine label="Payment ID" value={paymentId || 'N/A'} />
                        </div>
                        <div className='space-y-2'>
                            <DetailLine label="Payment Status" value={''} large={true} />
            ...
            <div className='flex justify-end gap-3 pt-4 border-t border-gray-200'>
                ...
                {/* Close Button is handled by the sticky top-right button */}
            </div>
